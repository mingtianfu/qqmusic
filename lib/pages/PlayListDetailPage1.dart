...
class PlayListDetailPage extends StatefulWidget {
  PlayListDetailPage({Key key, this.id, this.audioPlayer, this.handleTap}) : super(key: key);
  final int id;
  ...
  Widget _fixedBuild() {
    ...
    return Container(
      ...
      child: GestureDetector(
        onTap: () {
          inheritedContext.increment(songList, 0);
        },
        child: Row(
          children: <Widget>[
            Container(
  ...
  Widget _listBuild() {
    ...
    : new SliverFixedExtentList(
      ...
      delegate: new SliverChildBuilderDelegate(
        ...
        final num id = inheritedTestModel.songList.length == 0 ? 0 : inheritedTestModel.songList[inheritedTestModel.songListIndex].id;
        return Container(
          color: Colors.white,
          padding: EdgeInsets.only(right: 15, bottom: 10),
          child: GestureDetector(
            onTap: () {
              inheritedContext.increment(songList, index);
            },
            child: Row(
              children: <Widget>[
                Container(
                  width: 3,
                  height: 40,
                  color:  id == songList[index].id ? _selectColor : Colors.white,
                ),
                SizedBox(
                  width: 56,
                  height: 40,
      ...
      childCount: songList.length //50个列表项
      ),
    );
  }
