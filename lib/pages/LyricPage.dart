...
import 'package:qqmusic/utils/time.dart';
...
class LyricPage extends StatefulWidget {
  ...
  final Function(String positon) onTap;
...
class LyricState extends State<LyricPage> with TickerProviderStateMixin {
...
//            "but lyricPainter is unavaiable, so scroll(without animate) on next frame $d");
    ...
    // print(getTimeStamps((milliseconds/1000).toDouble()));
    // positon = getTimeStamps((milliseconds/1000).toDouble());
  ...
  _getPositon() {
    var totalHeight = lyricPainter.height; // 歌词的总高度
    var heihgt = totalHeight / lyricPainter.lyricPainters.length;
    var index = (-lyricPainter.offsetScroll / heihgt).ceil();
    // 当前行
    if (index < lyricPainter.lyricPainters.length) {
      _positon = lyricPainter.lyric[index]?.timeStamp;
    }
  }
    ...
    return Container(
      ...
      child: GestureDetector(
        ...
        onVerticalDragEnd: (details) async {
          // debugPrint("details.primaryVelocity : ${details.primaryVelocity}");
          await _getPositon();
...
//      painter.layout();//layout first, to get the height
    ...
    _styleHighlight = style.copyWith(color: Color(int.parse('31c27c',radix:16)|0xFF000000));
  }
