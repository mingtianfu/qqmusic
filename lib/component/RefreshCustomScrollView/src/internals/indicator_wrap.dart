  ...
  double _calculateScrollOffset() {
    return (floating
            ? (mode == RefreshStatus.twoLeveling ||
                ...
                ? RefreshCustomScrollView.ofState(context).viewportExtent
                : widget.height)
            : 0.0) -
        _position?.pixels;
  }
...
mixin IndicatorStateMixin<T extends StatefulWidget, V> on State<T> {
  RefreshCustomScrollView refresher;
  ...
  void _updateListener() {
    ...
    refresher = RefreshCustomScrollView.of(context);
  ...
  void initState() {
    // TODO: implement initState
    if (V == RefreshStatus) {
      RefreshCustomScrollView.of(context)?.controller?.headerMode?.value =
          RefreshStatus.idle;
    }
    super.initState();
  }
