...
class RefreshCustomScrollView extends StatefulWidget {
  ...
  RefreshCustomScrollView.builder(
      {Key key,
      @required this.controller,
  ...
  static RefreshCustomScrollView of(BuildContext context) {
    return context?.ancestorWidgetOfExactType(RefreshCustomScrollView);
  }
...
class SmartRefresherState extends State<RefreshCustomScrollView> {
  ...
  List<Widget> _buildSliversByChild(BuildContext context, Widget top, Widget child, RefreshConfiguration configuration) {
    List<Widget> slivers;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.builder != null) {
      ...
      List<Widget> slivers = _buildSliversByChild(context, widget.top, widget.child, configuration);
    ...
    return LayoutBuilder(builder: (c2, cons) {
...
class RefreshController {
  ...
  Future<void> requestRefresh(
    ...
    if (needMove)
      RefreshCustomScrollView.ofState(position.context.storageContext).setCanDrag(false);
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (needMove) {
        return Future.delayed(const Duration(milliseconds: 50)).then((_) async {
          await position
              ...
              ?.then((_) {
            RefreshCustomScrollView.ofState(position.context.storageContext)
                ?.setCanDrag(true);
